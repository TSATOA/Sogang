...
class LaneDetection:
    ...
    def cut_gray(self, state_image_full):
        ...
        '''
        gray_state_image = [96*[0] for _ in range(65)]
        
        for i in range(65):
            for j in range(len(state_image_full[i])):
                gray_state_image[i][j] = 0.299*state_image_full[i][j][0] + 0.587*state_image_full[i][j][1] + 0.114*state_image_full[i][j][2]
    ...
    def edge_detection(self, gray_image):
        ...
        gradient_sum = [96*[0] for _ in range(65)]
        ...
        for i in range(65):
            for j in range(len(gray_image[i])):
                ...
                for x in range(3):
                    for y in range(3):
                        if i+x-1<0 or i+x-1>64 or j+y-1<0 or j+y-1>len(gray_image[i])-1 :
                            pass
                        else:
                            suma += gx[x][y] * gray_image[i+x-1][j+y-1]
                            sumb += gy[x][y] * gray_image[i+x-1][j+y-1]
                sum = np.sqrt(suma*suma+sumb*sumb)
                sum = np.around(sum)
                if sum<self.gradient_threshold:
                    sum = 0
                gradient_sum[i][j] = sum
        for i in range(65):
            for j in range(len(gray_image[i])):
                if i == 0 or j == 0:
                    gradient_sum[i][j] = 0
    ...
    def find_maxima_gradient_rowwise(self, gradient_sum):
        ...
        argmax = []
        for i in range(1,64):
            peak, value = find_peaks(gradient_sum[i],distance = 3,height=100)
            argmax.append(peak)
        argmaxima = argmax
    ...
    def lane_detection(self, state_image_full):
        ...
        if lane_found:
            ...
            cn1 = 0
            cn2 = 0
            for idx,i in enumerate(maxima):
                for idy,j in enumerate(maxima[idx]):
                    if lane_boundary1_points[cn1][0] -5 <= maxima[idx][idy] and maxima[idx][idy] <= lane_boundary1_points[cn1][0]+5:
                        if cn1-3 < idx and idx < cn1+3: 
                            lane_boundary1_points = np.append(lane_boundary1_points,np.array([[maxima[idx][idy],idx]]),axis=0)
                            maxima[idx][idy] = 9999
                            cn1+=1
                    if lane_boundary2_points[cn2][0] -5 <= maxima[idx][idy] and maxima[idx][idy] <= lane_boundary2_points[cn2][0]+5:
                        if cn2-3 < idx and idx < cn2+3:
                            lane_boundary2_points = np.append(lane_boundary2_points,np.array([[maxima[idx][idy],idx]]),axis=0)
                            maxima[idx][idy] = 9999
                            cn2+=1
            
            for idx in range(len(maxima)-1,0,-1):
                for idy,j in enumerate(maxima[idx]):
                    if lane_boundary1_points[cn1][0] -5 <= maxima[idx][idy] and maxima[idx][idy] <= lane_boundary1_points[cn1][0]+5:
                        if cn1-3 < idx and idx < cn1+3: 
                            lane_boundary1_points = np.append(lane_boundary1_points,np.array([[maxima[idx][idy],idx]]),axis=0)
