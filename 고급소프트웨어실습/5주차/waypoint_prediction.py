...
def waypoint_prediction(roadside1_spline, roadside2_spline, num_waypoints=6, way_type = "smooth"):
    ...
    if way_type == "center":
        ...
        List2 = []
        for i in range(6):
            List1.append((roadside1_points[0][i]+ roadside2_points[0][i])/2)
            List2.append((roadside1_points[1][i]+ roadside2_points[1][i])/2)
        way_points = np.array([List1,List2])
        return way_points
        ...
        for i in range(6):
            List1.append((roadside1_points[0][i]+ roadside2_points[0][i])/2)
            List2.append((roadside1_points[1][i]+ roadside2_points[1][i])/2)
        way_points_center = np.array([List1,List2]).reshape(-1)
        ...
        ways = minimize(smoothing_objective,way_points_center,way_points_center)
...
def target_speed_prediction(waypoints, num_waypoints_used=5,
    ...
    target_speed = (max_speed - min_speed)*np.exp(-exp_constant * np.abs(6-2-curvature(waypoints))) + min_speed
