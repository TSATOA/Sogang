...
void checkPuzzle(int puzzle[][size]) 
{
	...
	for(i=0;i<size;i++){
		for(j=0;j<size;j++){
			if(puzzle[i][j]==size*size-1-size*i-j) cnt++;
		}
		if(cnt==size*size){
			game_over=1;
			return ;
...
{
    int r,l;
    int iter = 1000;     /* 필요시횟수를 변경해 가면서 테스트 해보세요 */
    while(iter)
    {
        iter--;
        l = find_0_loc(puzzle);
...
int main(void)
{
    ...
    {  
		...
		*/
		...
		loc=find_0_loc(puzzle);  /* 퍼즐게임판에서 빈칸을 찾는다. ( find_0_loc() )*/
		check=move_to(puzzle,loc,command);  /* key에 대한 동작을 수행한다.*/
		if(check) life--;	
		...
		if(life<=0) break;	
		checkPuzzle(puzzle); /*  퍼즐게임의 종료 조건을 체크한다.*/
