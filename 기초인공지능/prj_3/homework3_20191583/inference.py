...
def constructBayesNet(gameState: hunters.GameState):
    ...
    # Add variables, edges to list variables, edges
    ...
    for i in range(X_RANGE):
        for j in range(Y_RANGE):
            k = (i,j)
            x.append(k)
    variableDomainsDict[PAC] = x
    variableDomainsDict[GHOST0] = x
    variableDomainsDict[GHOST1] = x
    "*** END YOUR CODE HERE ***"
    y = []
    for i in range(X_RANGE+Y_RANGE+MAX_NOISE-1):
        y.append(i)
    variableDomainsDict[OBS0] = y
    variableDomainsDict[OBS1] = y
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        # inferenceByEnumeration 122line : Bring all factors including evidence
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)

        # Take one of the strings to be removed and join it.
        # When the joined factor has two or more condition variables
        # -> Add the factor from which the string has been removed to the unjoined factor list(ex elimination:A  A,B,C|D -> B,C|D Add list)
        # If less than 1, Adding to the unjoined factor list does not occur, so the factor to which the string to be removed belongs disappears.
        # Repeat this process, the strings to be removed will disappear.
        for i in eliminationOrder:
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList,i)
            if len(joinedFactor.unconditionedVariables())>1:
                currentFactorsList.append(eliminate(joinedFactor,i))
        # Normalize the completed factor and return
