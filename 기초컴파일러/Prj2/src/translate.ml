...
let rec process_args args symbol_table instrs =
  ...
  | (typ, arg_name) :: tail_args ->
    ...
    let new_instrs = instrs @ [LocalAlloc (alloc_reg, size); Store (Reg arg_name, alloc_reg)] in
    let new_sym_tbl = SymbolMap.add arg_name (typ, alloc_reg) symbol_table in
    process_args tail_args new_sym_tbl new_instrs
  ...
  | Arr (arr_name, idx_expr) ->
    ...
    if SymbolMap.mem arr_name symbol_table then
      ...
      | (CIntArr _, base_addr_reg) ->
        let base_addr_instrs = [Copy (base_addr_reg, addr_reg)] in
        let offset_instrs = [BinOp (f_reg, MulOp, Reg idx_reg, Imm (ImmInt 4))] in
        let faddr_instrs = [BinOp (f_reg, AddOp, Reg f_reg ,Reg addr_reg)] in
        let load_instr = [Load (reg, f_reg)] in
        (idx_instrs @ base_addr_instrs @ offset_instrs @ faddr_instrs @load_instr, reg)
      | (CBoolArr _, base_addr_reg) ->
        let base_addr_instrs = [Copy (base_addr_reg, addr_reg)] in
        let offset_instrs = [BinOp (f_reg, MulOp, Reg idx_reg, Imm (ImmInt 1))] in
        let faddr_instrs = [BinOp (f_reg, AddOp, Reg f_reg ,Reg addr_reg)] in
        let load_instr = [Load (reg, f_reg)] in
        (idx_instrs @ base_addr_instrs @ offset_instrs @ faddr_instrs @load_instr, reg)
      | _ -> failwith ("Array not found: " ^ arr_name)
    else
    ...
    let (instrs, updated_symbol_table) = match stmt with
      ...
      | Assign (lv, expr) ->
        ...
        let store_instrs = match lv with
          | LVar v -> 
            let (_,addr) = SymbolMap.find v symbol_table in
            [Store (Reg expr_reg, addr)]
          | LArr (arr_name, idx_expr) ->
            ...
            if SymbolMap.mem arr_name symbol_table then
              match SymbolMap.find arr_name symbol_table with
              | (CIntArr _, base_addr_reg) ->
                let offset_instr = BinOp (addr_reg, MulOp, Reg idx_reg, Imm (ImmInt 4)) in
                let addr_instr = BinOp (addr_reg, AddOp, Reg base_addr_reg, Reg addr_reg) in
                idx_instrs @ [offset_instr; addr_instr; Store (Reg expr_reg, addr_reg)]
              | (CBoolArr _, base_addr_reg) ->
                let offset_instr = BinOp (addr_reg, MulOp, Reg idx_reg, Imm (ImmInt 1)) in
                let addr_instr = BinOp (addr_reg, AddOp, Reg base_addr_reg, Reg addr_reg) in
                idx_instrs @ [offset_instr; addr_instr; Store (Reg expr_reg, addr_reg)]
