...
void InitTetris(){
	...
	*/
	...
	maxX=0;maxY=0;maxR=0;maxScore=0;
...
void BlockDown(int sig){
	// user code
	int i,j,t,k;
//	if(rec==1) drawChange(field,KEY_DOWN)
...
int modified_recommend(RecNode *root){
	...
	c=(RecNode**)malloc(sizeof(RecNode*)*s[nextBlock[root->lv]]);
	for(i=0;i<s[nextBlock[root->lv]];i++) c[i]=(RecNode*)malloc(sizeof(RecNode));
	...
	for(i=0;i<r[nextBlock[root->lv]];i++){
		for(j=-2;j<WIDTH+3;j++){
			...
			for(y=0;y<HEIGHT;y++)
				for(x=0;x<WIDTH;x++)
					c[k]->f[y][x]=root->f[y][x];
			c[k]->curBlockID=nextBlock[root->lv];
			c[k]->recBlockX=j;
			c[k]->recBlockY=0;
			c[k]->recBlockRotate=i;
			c[k]->parent=root;

			while(CheckToMove(c[k]->f,c[k]->curBlockID,c[k]->recBlockRotate,c[k]->recBlockY+1,c[k]->recBlockX)){
				c[k]->recBlockY++;
			}
			c[k]->score+=AddBlockToField(c[k]->f,c[k]->curBlockID,c[k]->recBlockRotate,c[k]->recBlockY,c[k]->recBlockX);
			c[k]->score+=DeleteLine(c[k]->f);
			
			if(c[k]->lv==VISIBLE_BLOCKS){
				if(c[k]->score>=maxScore&&maxY<=c[k]->recBlockY){
					maxScore=c[k]->score;
					a=c[k];
					while(a->lv!=1) a=a->parent;
					maxY=a->recBlockY;
					maxX=a->recBlockX;
					maxR=a->recBlockRotate;
				}
			}else{
				if(k==0)
					modified_recommend(c[k]);
				if(k>0&&c[k]->score>=c[k-1]->score)
					modified_recommend(c[k]);
			}
			c[k]->score=0;
...
void BlockDownRec(int sig){
	...
	if(blockY==0) gameOver=1;
	k=AddBlockToField(field,nextBlock[0],maxR,maxY,maxX);
	if(k!=0) score+=k;
	score+=DeleteLine(field);
	PrintScore(score);
	for(i=0;i<VISIBLE_BLOCKS-1;i++) nextBlock[i]=nextBlock[i+1];
	...
	DrawBlock(maxY,maxX,nextBlock[0],maxR,'R');
	DrawNextBlock(nextBlock);
	move(HEIGHT,WIDTH+10);
	}
	timed_out=0;
